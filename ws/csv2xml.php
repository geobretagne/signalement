	...
	function csv2xml($file, $epsg, $rows = 'row'){
     ...
     while (($data = @fgetcsv($handle, 2000, ';')) !== FALSE){
          ...
          for ($i = 0; $i < $cols; $i++){
			   ...
			   //traite le cas de la date (transformer le format 04/03/2015 en format ISO)
			   if ($titles[$i]=="date_saisie") {
			   $reponse .= str_replace(' ', '-', strtolower("<feature:{$titles[$i]}>"));
               
			   $date_iso = substr($data[$i],3,2)."/".substr($data[$i],0,2)."/".substr($data[$i],6);//inverser mois et jour pour le passer en format americain
			   $date_iso = date("c", strtotime($date_iso)); //conversion format ISO
			   $reponse .= $date_iso;
               $reponse .= str_replace(' ', '-', strtolower("</feature:{$titles[$i]}>"));	
			   }
